...
- Current repo state: real Chat SDK transport now exists for `/해줘`, `/뮤엘`, and prefixed `뮤엘 ...`; the public `/만들어줘` slash surface is retired; shared user-facing sanitization exists; the current `chat-sdk` owner path now has deployed-internal default-on/100 plus two-surface rollback evidence; remaining debt is grace-close and exact-unit legacy cleanup.
...
| --- | --- | --- | --- |
| Transport owner not fully closed | `src/discord/runtime/discordIngressAdapter.ts`, `src/discord/runtime/chatSdkRuntime.ts`, `src/discord/runtime/commandRouter.ts` | The eligible surfaces now have deployed-internal default-on/100 plus exact-unit rollback evidence for the current `chat-sdk` owner window, but the rollback grace window and legacy demotion/removal are not fully closed | Eligible surfaces run default-on through the extracted owner path with explicit rollback artifacts for each exact unit |
