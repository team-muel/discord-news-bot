...
- 사용자가 후속 공정도 모두 자율작업으로 두라고 명시하면, agent는 downstream stage를 단계별 재지시 없이 연속 실행하는 것을 기본 contract로 본다. 중단은 policy, destructive action, approval, architecture-boundary blocker가 생길 때만 허용한다.
