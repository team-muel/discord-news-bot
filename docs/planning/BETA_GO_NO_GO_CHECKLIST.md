...
8. `obsidian_adapter_routing == pass` (remote-mcp 우선 라우팅 확인, fallback 체인 정상)
...
- Gate 8 pass condition example:
  - remote-mcp adapter가 GCP VM MCP 서버에 정상 연결됨
  - fallback adapter chain(native-cli → script-cli → local-fs)이 정상 동작
