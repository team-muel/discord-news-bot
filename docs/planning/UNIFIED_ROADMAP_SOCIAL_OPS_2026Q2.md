...
## Phase D (D91-D120): Deep Integration Unlock

목표: 기존 외부 OSS adapter의 활용율을 5-25%→80%+로 끌어올리기

- D1. OpenJarvis Learning Loop 완결 [M-11]
  - `jarvis.bench --json` 결과를 `computeNormalizedQualityScore` 입력으로 연결
  - weekly auto-judge 후 `jarvis.optimize` 자동 트리거
  - trace→learning→bench 폐순환 검증 (2주 연속 bench score 회귀 없음)
- D2. OpenShell Sandbox 격리 실행 [M-12]
  - Docker Desktop WSL2 통합 또는 remote gateway 경로 확보
  - actionRunner에서 `implement.execute` → OpenShell sandbox 위임
  - `HIGH_RISK_APPROVAL_ACTIONS` ↔ OpenShell network policy YAML 동기화

완료 기준:

- jarvis bench 결과가 go/no-go gate에 자동 반영
- sandbox 외부 코드 실행 0건 (로그 검증)

## Phase E (D121-D150): Channel Expansion + Framework

목표: 채널 커버리지 확장 + 미래 도구 pluggable 대비

- E1. OpenClaw Gateway 채널 브릿지 [M-13]
  - `openclaw agent --message` 경로를 Muel↔OpenClaw 양방향 메시지 파이프로 확장
  - guild 설정에 채널 라우팅 매핑 (discord: native, whatsapp: openclaw, telegram: openclaw)
  - Muel feature lacuna → OpenClaw skill 자동 생성 트리거
- E2. Pluggable Adapter Framework [M-15]
  - `ExternalAdapterId` union literal → string 기반 + schema validation
  - `src/services/tools/adapters/` glob scan → 자동 registry 등록
  - adapter onboarding checklist 자동 생성 (`generate-onboarding-checklist.mjs` 확장)

완료 기준:

- Discord 메시지가 OpenClaw 세션으로 relay 성공
- 신규 adapter 등록 시 타입 파일 수정 불필요

## Phase F (D151+): Gap-to-Vision Closure

목표: Section 3 잔여 갭 해소 — 사회학적 구조화 고도화, 레이턴시 SLO 자동화, 코드 유지보수성 개선

- F1. User CRM 심화 + Social Graph 고도화 [M-19]
  - `userCrmService` UserProfile에 세그먼트/코호트 태그 자동 부여 (power_user/casual/dormant)
  - `communityGraphService`에 클러스터 탐지 추가 (relationship_edges 기반 connected component → community_clusters)
  - 에스컬레이션 패턴 탐지: 유저→관리자 멘션 빈도 급증 시 알림 (escalation_signals)
  - CRM snapshot에 social graph 요약 포함 (상위 관계 3명, 소속 클러스터)
- F2. LLM 레이턴시 SLO 자동 Fallback [M-20]
  - 위클리 리포트 p95가 LATENCY_P95_THRESHOLD_MS 초과 시 자동 provider 다운그레이드
  - p95 레이턴시를 go/no-go gate 판정 입력에 포함
  - 세션 품질 집계: 기존 quality_score 기반 SQL view (코드 구현 최소화)
- F3. 서비스 모듈 유지보수성 개선 [M-21]
  - 잔여 root-level 서비스 파일 → 하위 디렉토리 이동
  - 순환 의존성 탐지 + 해소
  - 서비스 간 계약을 interface 파일로 분리 (types/ 디렉토리)
- F4. 외부 OSS 어댑터 활용률 80%+ [M-22]
  - 등록된 adapter 중 실제 호출 비율 tracking
  - 비활성 어댑터 자동 health-check + 재활성화 제안 루프
  - adapter probe 결과를 weekly 리포트에 포함
- F5. 운영 문서 통합 경량화 [M-23]
  - canonical 4문서 이외 잔여 문서 → archive 또는 canonical 흡수
  - RUNBOOK/SOP Decision Table 중복 해소
  - Obsidian vault backlink 정합성 검증 자동화
  - Obsidian operating system blueprint/object model/transition plan를 기준 문서로 고정
- F6. 채널 ingress 추상화 + Chat SDK 준비 [M-24]
  - Discord/OpenClaw ingress를 첫 adapter로 유지하되 channel ingress normalization contract로 분리
  - future Chat SDK adapter가 같은 routing/policy contract를 재사용하도록 request envelope를 고정
  - Hermes는 경쟁하는 직접 채팅 ingress가 아니라 continuity/operator lane으로 유지

완료 기준:

- Gap 1 (사회학적 구조화): 클러스터/에스컬레이션 KPI 2개 이상 계측 가능
- Gap 3 (품질 계측): p95 SLO 위반 시 자동 fallback 동작 확인
- 순환 의존성 0건, adapter 활용률 80%+, 운영 문서 3개 이하
- 기존 Discord/OpenClaw ingress 동작을 유지한 채 future Chat SDK adapter 삽입 지점과 ownership boundary가 문서화됨
...
- M-09: External Tool Integration (Phase 1-5)
- M-10: Codebase Health & Infrastructure (tech debt, naming, CI stability)
- M-11: OpenJarvis Learning Loop (trace→optimize→bench 폐순환, 로컬 모델 자동 개선)
- M-12: OpenShell Sandbox Isolation (implement.execute 격리 실행, 정책 기반 credential 관리)
- M-13: OpenClaw Gateway Channel Bridge (Discord→멀티채널 확장, skill 연동)
- M-15: Pluggable Adapter Framework (동적 adapter 등록, FinTech 등 미래 도구 대비)
- M-19: User CRM 심화 + Social Graph 고도화 (코호트 세그먼트, 클러스터 탐지, 에스컬레이션 신호)
- M-20: LLM 레이턴시 SLO 자동 Fallback (p95 gate 연동, 자동 provider 다운그레이드)
- M-21: 서비스 모듈 유지보수성 개선 (순환 의존성 해소, 디렉토리 재구성 2차)
- M-22: 외부 OSS 어댑터 활용률 80%+ (활용률 계측, health-check 자동화)
- M-23: 운영 문서 통합 경량화 (canonical 흡수, 중복 해소, backlink 검증)
- M-24: 채널 ingress 추상화 (Discord/OpenClaw 우선, Chat SDK 준비, Hermes 상호보완 유지)
