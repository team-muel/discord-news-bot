...
-- ==========================================
-- 9. Automation worker tables (required by Python jobs)
-- ==========================================
...
create index if not exists idx_news_sentiment_sector_created_at
  on public.news_sentiment (sector, created_at desc);

create table if not exists public.youtube_log (
...
drop trigger if exists trg_youtube_log_updated_at on public.youtube_log;
create trigger trg_youtube_log_updated_at
before update on public.youtube_log
for each row
