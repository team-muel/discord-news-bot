...
create table if not exists public.users (
  ...
  avatar text,
  discord_access_token text,  -- AES-256-GCM encrypted (enc:v1:… prefix)
  discord_refresh_token text, -- AES-256-GCM encrypted (enc:v1:… prefix)
...
-- Atomic lease acquisition — replaces TOCTOU-vulnerable UPDATE → INSERT pattern.
...
insert into public.agent_skill_catalog (
  ...
  ('*', 'casual_chat', true, '일상 대화 응답', '감정 표현/잡담 입력을 공감형 자연어로 응답합니다.', '감정 표현, 안부, 짧은 일상 발화', '도구 없이 공감 + 필요 시 1개 되묻기', '너는 공감형 한국어 대화 파트너다. 작업 실행/검색 도구 호출 없이 자연스럽게 반응한다. 감정/짧은 일상 발화는 장기기억 조회보다 현재 공감과 가벼운 질문을 우선한다.', 'casual_chat', false, 0.5, 220, 10),
  ('*', 'ops-plan', true, '운영 계획 수립', '목표를 실행 가능한 단계로 분해하고 우선순위를 제안합니다.', '운영 목표, 제한사항, 기간, 성공조건', '단계별 실행계획 + 실패시 대안 + 우선순위', '너는 디스코드 서버 운영 자동화 계획가다. 한국어로 간결하고 실행 가능한 계획만 제시한다. 추측이나 단정은 피하고, 관측 가능한 사실 중심으로 작성한다.', 'ops-plan', false, 0.2, 900, 20),
  ('*', 'ops-execution', true, '운영 실행안 생성', '계획을 실제 운영자가 사용할 수 있는 체크리스트로 변환합니다.', '목표 + 계획안 + 리소스 제약', '즉시 실행 체크리스트 + 자동화 포인트 + 관찰 지표', '너는 서버 운영 실행 담당 에이전트다. 운영자가 바로 실행할 수 있는 단계와 확인 포인트를 제공한다. 모호한 조언보다 명확한 액션 아이템을 우선한다.', 'ops-execution', false, 0.25, 1000, 30),
  ('*', 'ops-critique', true, '운영 리스크 검토', '실행안의 리스크와 보완책을 검토합니다.', '목표 + 실행안', '리스크 목록 + 완화안 + 즉시 적용 가드레일', '너는 운영 품질/보안 검토 에이전트다. 개인정보, 권한, 비용, 운영중단 위험을 우선 검토한다. 위험 지적만 하지 말고 실행 가능한 보완안까지 제공한다.', 'ops-critique', false, 0.1, 800, 40),
  ('*', 'guild-onboarding-blueprint', true, '길드 온보딩 설계', '서버 초대 후 온보딩 절차와 동의 기반 학습 플로우를 설계합니다.', '서버 성격, 권한 정책, 수집 범위', '온보딩 상태머신 + 동의 UX + 초기 데이터 수집 전략', '너는 디스코드 길드 온보딩 아키텍트다. opt-in, 데이터 최소수집, 관리자 제어권을 기본 원칙으로 둔다.', 'guild-onboarding-blueprint', false, 0.2, 1200, 50),
  ('*', 'incident-review', true, '장애/오답 회고', '장애나 오답 사례를 회고하고 재발 방지 규칙을 도출합니다.', '사건 요약, 영향, 현재 대응', '원인 가설 + 검증 절차 + 재발 방지 체크리스트', '너는 운영 회고 에이전트다. 비난이 아닌 사실 기반 원인 분석과 재발 방지에 집중한다.', 'incident-review', true, 0.15, 900, 60),
  ('*', 'webhook', true, '웹훅 설계/운영', '웹훅 이벤트 계약, 검증, 재시도, 운영 가드레일까지 포함한 실행안을 생성합니다.', '이벤트 종류, 공급자, 인증 방식, 처리 목표, 실패 정책', '엔드포인트 계약 + 검증/보안 + 재시도/멱등성 + 운영 체크리스트', '너는 웹훅 통합 아키텍트다. 이벤트 계약과 보안 검증(HMAC/서명), 멱등성 키, 재시도 전략을 필수로 포함한다. 실제 운영자가 바로 적용 가능한 단계와 예시를 한국어로 제공한다.', 'webhook', false, 0.2, 1200, 70)
...
values
  ('*', 'fast', true, '[{"role":"planner","title":"목표 실행 계획 수립","skipWhenFast":true,"skipWhenRequestedSkill":false},{"role":"researcher","title":"실행의 근거 초안 작성","skipWhenFast":false,"skipWhenRequestedSkill":true},{"role":"critic","title":"리스크 검토 및 보완","skipWhenFast":true,"skipWhenRequestedSkill":true}]'::jsonb),
  ('*', 'balanced', true, '[{"role":"planner","title":"목표 실행 계획 수립","skipWhenFast":false,"skipWhenRequestedSkill":false},{"role":"researcher","title":"실행의 근거 초안 작성","skipWhenFast":false,"skipWhenRequestedSkill":true},{"role":"critic","title":"리스크 검토 및 보완","skipWhenFast":false,"skipWhenRequestedSkill":true}]'::jsonb),
  ('*', 'precise', true, '[{"role":"planner","title":"목표 실행 계획 수립","skipWhenFast":false,"skipWhenRequestedSkill":false},{"role":"researcher","title":"실행의 근거 초안 작성","skipWhenFast":false,"skipWhenRequestedSkill":true},{"role":"critic","title":"리스크 검토 및 보완","skipWhenFast":false,"skipWhenRequestedSkill":true}]'::jsonb)
...
values (
  ...
  '[
    {
        "pattern": "(이메일|전화번호|연락처|주소|주민등록|신분증|여권|계좌|카드번호|토큰|비밀번호|api key)",
        "score": 28,
        "reason": "personal_or_secret_identifier"
    },
    {
        "pattern": "(실명|본명|생년월일|birthday|dob|주번|학번)",
        "score": 20,
        "reason": "identity_attribute"
    },
    {
        "pattern": "(환자|의료|진단|건강기록|병력|상담기록)",
        "score": 24,
        "reason": "health_sensitive_domain"
    },
    {
        "pattern": "(청소년|미성년|학생 개인정보|아동)",
        "score": 24,
        "reason": "minor_sensitive_context"
    }
]'::jsonb,
  '[
    {
        "pattern": "(원본.*(대량 공유|전송)|전체.*덤프|raw.*log|전건 다운 파일)",
        "score": 55,
        "reason": "bulk_sensitive_export"
    },
    {
        "pattern": "(토큰.*보여|비밀번호.*출력|패스워드.*공개|credentials?.*dump)",
        "score": 60,
        "reason": "secret_exposure_attempt"
    },
    {
        "pattern": "(개인정보.*수집.*자동|동의 없이|무단 수집)",
        "score": 50,
        "reason": "non_consensual_collection"
    }
]'::jsonb
)
on conflict (guild_id) do nothing;
