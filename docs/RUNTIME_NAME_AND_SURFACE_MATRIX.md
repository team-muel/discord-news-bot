...
| NVIDIA NIM remote reasoning via LiteLLM | LLM provider layer | litellm.config.yaml `muel-nemotron`, `muel-gemma4`, `muel-qwen-coder`, `muel-qwen-coder-max`, `muel-nemotron-ultra` entries, LiteLLM proxy on :4000 | LiteLLM proxy model routing + `/health/liveliness` probe | no | NVIDIA NIM API key required | implemented for opt-in remote reasoning/code lanes |
