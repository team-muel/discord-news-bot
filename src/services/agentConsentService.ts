...
const normalizeSnapshot = (row: Record<string, unknown>, fallback: AgentUserConsentSnapshot): AgentUserConsentSnapshot => ({
  ...
  socialGraphEnabled: row.social_graph_enabled !== false,
  profilingEnabled: row.profiling_enabled !== false,
  actionAuditDisclosureEnabled: row.action_audit_disclosure_enabled !== false,
