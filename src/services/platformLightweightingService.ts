  ...
  const items: LightweightingItem[] = [
    {
      ...
      priority: 'high',
      current: 'login-session and maintenance cleanup can be db-owned via pg_cron with app loop fallback toggle',
      ...
      files: [
        ...
        'src/services/supabaseExtensionOpsService.ts',
      ],
    ...
    },
    {
      ...
      priority: 'high',
      current: 'memory search is rpc-first with automatic classic fallback on rpc failure',
      target: 'keep one rpc-first pipeline and use classic path only for resilience fallback',
      files: ['src/services/agentMemoryStore.ts'],
      readiness: pgTrgm ? 'ready' : 'blocked',
      blockers: pgTrgm ? [] : ['pg_trgm extension is not installed'],
    },
    {
      id: 'diag-001',
    ...
    },
    {
      ...
      category: 'timer-consolidation',
      priority: 'medium',
      current: 'each domain loop has isolated timer and enable flags',
      target: 'single policy map for loop ownership and startup wiring',
      files: [
        'src/discord/runtime/readyWorkloads.ts',
        'src/services/agentOpsService.ts',
        'src/services/retrievalEvalLoopService.ts',
        'src/services/runtimeSchedulerPolicyService.ts',
      ],
