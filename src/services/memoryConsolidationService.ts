...
const CONSOLIDATION_ENABLED = parseBooleanEnv(process.env.MEMORY_CONSOLIDATION_ENABLED, true);
const CONSOLIDATION_INTERVAL_MS = Math.max(
  60_000,
  parseIntegerEnv(process.env.MEMORY_CONSOLIDATION_INTERVAL_MS, 6 * 60 * 60_000), // 6h default
);
const CONSOLIDATION_MIN_GROUP_SIZE = Math.max(2, parseIntegerEnv(process.env.MEMORY_CONSOLIDATION_MIN_GROUP_SIZE, 3));
const CONSOLIDATION_MAX_BATCH = Math.max(1, parseIntegerEnv(process.env.MEMORY_CONSOLIDATION_MAX_BATCH, 5));
const CONSOLIDATION_RAW_AGE_HOURS = Math.max(1, parseIntegerEnv(process.env.MEMORY_CONSOLIDATION_RAW_AGE_HOURS, 6));
