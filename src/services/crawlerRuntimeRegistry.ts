...
type SourceUpdatePayload = Partial<
  Pick<Source, 'last_check_status' | 'last_check_error' | 'last_check_at' | 'last_post_signature' | 'name'>
>;
...
type SupabaseLike = {
  from: (table: string) => {
    ...
    update: (payload: SourceUpdatePayload) => any;
    delete: () => any;
  };
};
...
export type CrawlerRuntimeRegistryDeps = {
  ...
  logEvent: (message: string, type: 'info' | 'error' | 'success', userId?: string) => Promise<unknown>;
...
export const createCrawlerRuntimeRegistry = (deps: CrawlerRuntimeRegistryDeps) => {
  const processSource = async (source: Source) => {
    ...
    try {
      ...
      const updateData: SourceUpdatePayload = {
  ...
  const getSources = async (req: AuthenticatedRequest, res: Response) => {
    if (!deps.isSupabaseConfigured) {
      return res.json({ sources: [], total: 0, page: 1, limit: deps.defaultPageLimit });
    }

    try {
      const page = Math.max(1, parseInt(req.query.page as string, 10) || 1);
      const limit = Math.min(deps.defaultPageLimit, parseInt(req.query.limit as string, 10) || deps.defaultPageLimit);
  ...
  const addSource = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      const { url, name, guildId, channelId, guildName, channelName } = req.body as {
        url?: string;
        name?: string;
        guildId?: string;
        channelId?: string;
      ...
      if (count !== null && count >= deps.maxSourcesPerGuild) {
        return res.status(403).json({
          error: `해당 서버에는 최대 ${deps.maxSourcesPerGuild}개까지만 알림을 등록할 수 있습니다. (추후 프리미엄 기능으로 해금 예정 🚀)`,
        });
      }
  ...
  const triggerTest = async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { url, channelId } = req.body as { url?: string; channelId?: string };
