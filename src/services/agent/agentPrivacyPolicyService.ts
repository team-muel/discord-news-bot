...
const DEFAULT_REVIEW_RULES: AgentPrivacyRuleInput[] = [
  { pattern: '(?´ë©”???„í™”ë²ˆí˜¸|?°ë½ì²?ì£¼ì†Œ|ì£¼ë??±ë¡|? ë¶„ì¦??¬ê¶Œ|ê³„ì¢Œ|ì¹´ë“œë²ˆí˜¸|? í°|ë¹„ë???api key)', score: 28, reason: 'personal_or_secret_identifier' },
  { pattern: '(?¤ëª…|ë³¸ëª…|?ë…„?”ì¼|birthday|dob|?™ë²ˆ|?¬ë²ˆ)', score: 20, reason: 'identity_attribute' },
  { pattern: '(?˜ìž|?˜ë£Œ|ì§„ë‹¨|ê±´ê°•ê¸°ë¡|ë³‘ë ¥|?ë‹´ê¸°ë¡)', score: 24, reason: 'health_sensitive_domain' },
  { pattern: '(ì²?†Œ??ë¯¸ì„±???™ìƒ ê°œì¸?•ë³´|?„ë™)', score: 24, reason: 'minor_sensitive_context' },
];

const DEFAULT_BLOCK_RULES: AgentPrivacyRuleInput[] = [
  { pattern: '(?ë³¸.*(?´ë³´??ê³µìœ |?„ì†¡)|?„ì²´.*?¤í”„|raw.*log|?€???ë¬¸ ?„ë?)', score: 55, reason: 'bulk_sensitive_export' },
  { pattern: '(? í°.*ë³´ì—¬|ë¹„ë???*ì¶œë ¥|?¨ìŠ¤?Œë“œ.*ê³µê°œ|credentials?.*dump)', score: 60, reason: 'secret_exposure_attempt' },
  { pattern: '(ê°œì¸?•ë³´.*?˜ì§‘.*?ë™|?™ì˜ ?†ì´|ë¬´ë‹¨ ?˜ì§‘)', score: 50, reason: 'non_consensual_collection' },
];
