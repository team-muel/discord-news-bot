...
export const triggerGuildOnboardingSession = (params: {
  ...
  if (hasRecentOnboardingSession(params.guildId)) {
    return { ok: false, message: 'ìµœê·¼ ?¨ë³´???¸ì…˜???´ë? ì¡´ìž¬?©ë‹ˆ??' };
  }
  ...
  const goal = [
    ...
    'ì¶œë ¥: ?´ì˜ ?±ê²© ?”ì•½, ì´ˆê¸° ê¶Œìž¥ ?ë™??3ê°? ?°ì´???˜ì§‘ ?™ì˜ UX, ?„í—˜?”ì†Œ',
  ].join('\n');
  ...
  return { ok: true, message: '?¨ë³´???¸ì…˜???œìž‘?ˆìŠµ?ˆë‹¤.', sessionId: session.id };
};
...
const runDailyLearningForGuildIds = (guildIds: string[]) => {
  ...
  for (const guildId of guildIds.slice(0, AGENT_DAILY_MAX_GUILDS)) {
    const goal = [
      '?¼ì¼ ?´ì˜ ?™ìŠµ/?Œê³  ?¤í–‰',
      `guildId=${guildId}`,
      'ëª©í‘œ: ìµœê·¼ ?´ì˜ ?´ìŠˆ, ?˜ëª»??ê°€?? ? ì? ? í˜¸ ë³€?? ?¬ë°œ ë°©ì? ì²´í¬ë¦¬ìŠ¤?¸ë? ?”ì•½',
      'ì¶œë ¥: 1) ?¤ëŠ˜???µì‹¬?™ìŠµ 2) ?˜ì •??ê·œì¹™ 3) ?´ì¼???°ì„ ê³¼ì œ',
    ].join('\n');
...
export const triggerDailyLearningRun = (client: Client, guildId?: string) => {
  ...
  return {
    ok: true,
    message: `?¼ì¼ ?™ìŠµ ?¤í–‰ ?”ì²­ ?„ë£Œ: ${result.summary}`,
    ...result,
  };
};
