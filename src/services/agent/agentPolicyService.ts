...
export const validateAgentSessionRequest = (params: {
  ...
  if (!goal) {
    return { ok: false, message: 'Î™©ÌëúÍ∞Ä ÎπÑÏñ¥ ?àÏäµ?àÎã§.' };
  }

  if (goal.length > snapshot.maxGoalLength) {
    return {
      ok: false,
      message: `Î™©Ìëú Í∏∏Ïù¥Í∞Ä ?àÎ¨¥ ÍπÅÎãà?? ÏµúÎ? ${snapshot.maxGoalLength}?êÍπåÏßÄ ?àÏö©?©Îãà??`,
    };
  }
  ...
  if (params.runningSessions >= snapshot.maxConcurrentSessions) {
    return {
      ok: false,
      message: `?ôÏãú ?§Ìñâ ?∏ÏÖò ?úÎèÑÎ•?Ï¥àÍ≥º?àÏäµ?àÎã§. ?ÑÏû¨ ?úÎèÑ: ${snapshot.maxConcurrentSessions}`,
    };
  }
  ...
  if (params.requestedSkillId && restrictedSkills.has(params.requestedSkillId) && !params.isAdmin) {
    return {
      ok: false,
      message: `?§ÌÇ¨ ${params.requestedSkillId}?Ä Í¥ÄÎ¶¨Ïûê ?ÑÏö©?ÖÎãà??`,
    };
  }
